...
export default function SignUp() {
    ...
    const handleSubmit = async () => {
        ...
        if (step === 0) {
            ...
            await axios.get(import.meta.env.VITE_BASE_API + '/api/checkcreds', {
                params: {
                    username,
                    email
                }
            ...
            <div className="sign__content">
                ...
                <div className="sign__fields">
                    {
                        ...
                        (
                            ...
                            <div className="sign__button">
                                ...
                                <motion.input value={username} whileFocus={{ scale: 1.1 }} className="sign__field" required type="text" placeholder="Enter Username" onChange={(e) => setUsername(e.target.value)}/>
                            </div>
                            <div className="sign__button">
                                <div className="sign__field-screen"></div>
                                <motion.input value={email} whileFocus={{ scale: 1.1 }} className="sign__field" required type="email" placeholder="Enter E-mail" onChange={(e) => setEmail(e.target.value)}/>
                            </div>
                            </>
                        ) :
                        (
                            <>
                            <div className="sign__button">
                                <motion.input value={password} whileFocus={{ scale: 1.1 }} className="sign__field" required type="password" placeholder="Enter Password" onChange={(e) => setPassword(e.target.value)} />
                                <div className="sign__field-screen"></div>
                            </div>
                            <div  className="sign__button">
                                <motion.input value={steamID} whileFocus={{ scale: 1.1 }} className="sign__field" required type="text" placeholder="Enter SteamID" onChange={(e) => setSteamID(e.target.value)} />
                                <div className="sign__field-screen"></div>
                            </div>
                            </>
                        )
                ...
                <div className="sign__submit">
                    <p className="sign__submit-text">Already have an account? <Link to="/login" className="sign__submit-text--pink">Log in</Link></p>
                    {
                        ...
                        >
                            <Button text="Back" onClick={loading ? () => {} : () => setStep(0)}/>
                        </motion.div>
                    }  
                    <motion.div
                        animate={
